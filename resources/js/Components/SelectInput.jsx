...
export default forwardRef(function SelectInput(
    { className = '', isValid=false, isFocused = false, keys="id", values="value", options={}, labelText='', labelClasses='', ...props },
    ref,
) {
    ...
    return (
        <div className='relative'>
            ...
            { options?.map(option => {
                return <option key={`opt-${option[keys] || option}`} value={option[keys] || option}>{option[values] || option}</option>
            }) }
            </select>
