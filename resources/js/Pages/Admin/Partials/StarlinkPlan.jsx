...
const StarlinkPlan = ({ oldData, isps, setOpen = () => {} }) => {

    const { data, setData, processing, post, put } = useForm({
        name: oldData?.name || '',
        category: oldData?.category || '30',
    ...
    const handleSubmit = (e) => {
        ...
        if ( ! oldData?.id )
            post(route('admin.starlink'), options);
        else
            put(route('admin.starlink.update', [oldData.id]), options);
    }
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-6">
                <div className="form-group">
                    <SelectInput
                        ...
                        values="name"
                        value={data.category}
                        options={[{name: 'BUSINESS'}, {name: 'PERSONAL'}]}
